...
import com.abc.supplier.exception.UserNotFoundException;
...
import com.abc.supplier.resource.ValidateResource;
...
public class BaseResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
		...
		try {
			...
			case "supplierResource":
				SupplierResource supplierResource = new SupplierResource();
				for (FieldError error : ex.getBindingResult().getFieldErrors()) {
					sField = supplierResource.getClass().getDeclaredField(error.getField());
					sField.setAccessible(true);
					sField.set(supplierResource.getClass().cast(supplierResource), error.getDefaultMessage());
				}
				return new ResponseEntity<>(supplierResource, HttpStatus.UNPROCESSABLE_ENTITY);			
			default:
